...
-- This migration keeps internal identifiers (e.g., is_suna_default, RPC names, routes) unchanged.
    ...
    IF NEW.personal_account = TRUE THEN
        ...
        IF v_rowcount > 0 THEN
            INSERT INTO public.credit_ledger (
                ...
                v_initial_credits,
                'tier_grant',
                'Welcome to SuperAgent! Free tier initial credits'
            );
...
COMMENT ON FUNCTION public.get_user_account_by_email(text) IS
'Gets user account by email address. Used by admin scripts to install SuperAgent agents.';
