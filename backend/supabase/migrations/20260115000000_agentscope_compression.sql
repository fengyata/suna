...
ALTER TABLE public.messages ADD COLUMN IF NOT EXISTS is_compressed BOOLEAN DEFAULT FALSE;
...
ON public.messages(thread_id, is_compressed) 
...
-- Grant permissions
GRANT ALL PRIVILEGES ON TABLE public.thread_compression_summaries TO authenticated, service_role;

-- Comment
COMMENT ON TABLE public.thread_compression_summaries IS 'Stores AgentScope memory compression summaries for each thread. When conversation exceeds token threshold, older messages are summarized and this summary is prepended to context.';
COMMENT ON COLUMN public.messages.is_compressed IS 'TRUE if this message has been compressed (summarized) by AgentScope and should be excluded from direct LLM context. Original content is preserved for expand_message tool.';
