    ...
    async def _deduct_credits(self, user_id: str, num_results: int, thread_id: Optional[str] = None) -> bool:
        ...
        try:
            ...
            success = await token_service.deduct_for_tool(
                ...
                cost_usd=total_cost,
                feat_id=FeatIdConfig.PEOPLE_SEARCH,
                message_id=message_id
            )
            
            if success:
                token_value = token_service.usd_to_token(total_cost)
                logger.info(f"Deducted {token_value} tokens (${total_cost:.2f}) for people search ({num_results} results)")
                return True
            else:
                logger.warning(f"Failed to deduct tokens for people search")
                return False
    ...
    async def people_search(
        ...
        try:
            ...
            if config.ENV_MODE == EnvMode.LOCAL and not token_service.billing_enabled:
                logger.info("Running in LOCAL mode - billing skipped for people search")
